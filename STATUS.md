...
### REQUIRED — Supabase schema, not yet run

Confirmed via a direct REST query before writing any code that `side_matches` does not
exist yet (`PGRST205: Could not find the table 'public.side_matches'`) — same kind of gap
as the `ctp_mode`/etc. columns from a few sessions back. Side match player-tag updates
work fine without this (they write to the existing `players` table), but the side match
record itself can't be saved — commit fails gracefully with a toast telling the TD to
tell Rob — until this runs. Run in the Supabase SQL editor
(https://supabase.com/dashboard/project/mewwizubdwfgvrhiylur/sql/new):

```sql
CREATE TABLE IF NOT EXISTS side_matches (
...
New `side-matches.js`. "Start Side Match" (added to the Match page and the History
page's Side Matches tab) opens a bottom-sheet modal, three steps:

1. Pick 2+ players as tappable pill chips from the full roster (same interaction as
   Diablo's player grid, but green-themed to match the rest of the app, not Diablo red).
2. Pick a course — the existing dropdown list, free-text "Other", or leave unspecified.
3. Enter each player's total score (hole-by-hole is optional per spec and not built this
   round — total score entry only, matching the spec's stated minimum).

Tag redistribution (`calculateSideMatchResults`) is the exact same logic as
`calculateResults()` in match.js: sort by score, ties broken by the better (lower)
incoming tag, then hand the sorted incoming tags back in that order so the best score
gets the lowest tag actually brought to the match. No ace pool or CTP involvement, per
spec.

`commitSideMatch()` updates each player's `tag`, `last_change`, and `tag_history` in
Supabase (confirmed `tag_history` is a real jsonb column on `players` already — it was
just never written anywhere in the app before now) the same way `commitResults()` does
for official matches, then inserts one row into `side_matches`. `match_count`/`best_tag`
are deliberately left untouched — the task's instructions for this feature only named tag
+ tag_history + last_change, and side matches are framed in the spec as informal/separate
from official stats.

### History page

The Side Matches tab now loads and renders real `side_matches` rows (date, course or
"Unspecified", player count, winner) with the same tap-to-expand full-results view as
Official Matches — same up/down arrow convention, reused rendering pattern. A missing
table is treated identically to zero rows, so the page still shows "No side matches
recorded yet." plus the Start button rather than breaking. Expand-state keys are now
prefixed (`'o'+id` / `'s'+id`) so official and side match ids can never collide.

Script tag added to tags.html (after history.js). No new page/nav needed — the flow is
modal-based from two existing pages, so `showPage()` didn't need a new branch.

Commit: `feat: side matches with tag redistribution and history`

### Verification

No test suite exists for this app; `side_matches` doesn't exist yet so a real end-to-end
commit isn't possible until the SQL above runs. Verified what could be verified:

- Opened the modal from both entry points via real clicks. Picked Kevin Bankson (tag 13)
  and Matej Branc (tag 17), gave Matej the better score — calculation correctly assigned
  Matej tag 13 and Kevin tag 17 (best score gets the lowest of the two tags brought to
  the match), matching the official redistribution exactly.
- Ran the full commit with `players.update` and `side_matches.insert` stubbed: correct
  update payloads for both players (tag, last_change, tag_history all correct), correct
  insert payload, in-memory state matched expectations, and confirmed afterward that the
  real Kevin/Matej tags in Supabase were unaffected.
- Separately let the real (missing) `side_matches` insert through with only
  `players.update` stubbed, confirming the catch path fires and shows the "ask Rob to run
  the setup SQL" toast rather than crashing.
- History page's Side Matches tab showed the Start button and placeholder correctly
  against the missing table.
- App loads with zero console errors in a fresh tab. The one console error seen during
  testing was from deliberately exercising the missing-table path on the History page
  (an unavoidable browser-logged 404, same as the pre-migration CTP situation) — confirmed
  separately, in a clean tab, that normal app load (Home and Match pages) shows nothing.
